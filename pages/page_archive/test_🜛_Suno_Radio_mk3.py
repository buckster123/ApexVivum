...
# Paths
AUDIO_FOLDER = os.getenv("SUNO_AUDIO_FOLDER", "generated_music")
...
def init_session_state():
    defaults = {
        ...
        'history_page': 0,
        'history_per_page': 20,
        'last_generated_tracks': None,  # NEW: Persist generated tracks
        'last_task_id': None,           # NEW: Persist task ID
    }
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
def convert_to_wav(api_key: str, clip_id: str) -> str:
    ...
    logger.info(f"Converting clip {clip_id} to WAV")
    
    try:
        response = requests.post(
            f"{base_url}/convert-to-wav-format",
            headers=headers,
            json=payload,
            timeout=20
        )
        ...
        if response.status_code != 200:
            logger.warning(f"WAV conversion HTTP {response.status_code}: {response.text[:200]}")
            return None
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
def check_wav_status(api_key: str, wav_task_id: str) -> dict:
    ...
    try:
        response = requests.get(
            f"{base_url}/get-wav-conversion-details",
            headers=headers,
            params={"taskId": wav_task_id},
            timeout=10
        )
        ...
        logger.error(f"WAV status check error: {str(e)}")
        return {"status": "ERROR", "error": str(e)}
...
def main():
    ...
    with tab1:
        ...
        # Format selection
        output_format = st.radio("Output Format", ["MP3", "WAV"], index=0, horizontal=True)
        
        # NEW: Manual reset button for generated tracks
        col_reset1, col_reset2 = st.columns([3, 1])
        with col_reset2:
            if st.button("🔄 Clear Results", type="secondary"):
                st.session_state.last_generated_tracks = None
        ...
        if st.button("🚀 Generate Music", type="primary", use_container_width=True):
            ...
            try:
                # Clear previous results when generating new music
                st.session_state.last_generated_tracks = None
                st.session_state.last_task_id = None
                ...
                with st.spinner("🚀 Submitting request..."):
                    ...
                    st.session_state.last_task_id = task_id  # Store task_id in session state
                ...
                # Store results in session state to survive re-runs
                st.session_state.last_generated_tracks = suno_data
        ...
        # ==================== FIX: Display persisted results ====================
        # FIX: Display last generated tracks from session state, surviving re-runs
        if st.session_state.last_generated_tracks:
            st.markdown("### 🎧 Generated Tracks")
            
            # Show task info
            if st.session_state.last_task_id:
                st.caption(f"Task ID: {st.session_state.last_task_id[:16]}...")
            
            for i, track in enumerate(st.session_state.last_generated_tracks):
                clip_id = track.get('id')
                track_id = f"{st.session_state.last_task_id}_{i}"
                ...
                with col_track:
                    ...
                    st.caption(f"Duration: {duration:.1f}s | Model: {model} | Format: {output_format}")
                
                # Determine final audio URL (MP3 vs WAV)
                final_audio_url = audio_url
                final_ext = "mp3"
                
                if output_format == "WAV":
                    with st.spinner("Converting to WAV..."):
                        wav_task_id = convert_to_wav(st.session_state.api_key, clip_id)
                        if wav_task_id:
                            while True:
                                wav_status = check_wav_status(st.session_state.api_key, wav_task_id)
                                if wav_status.get('status') == 'SUCCESS':
                                    final_audio_url = wav_status.get('audioUrl', audio_url)
                                    final_ext = "wav"
                                    break
                                elif wav_status.get('status') == 'ERROR':
                                    st.warning("WAV conversion failed, using MP3")
                                    break
                                time.sleep(3)
                        else:
                            st.warning("WAV conversion failed, using MP3")
                ...
                filename = f"{safe_title}_{clip_id[-8:]}.{final_ext}"
                filepath = None
                
                if st.session_state.auto_save:
                    try:
                        filepath = download_file(final_audio_url, Path(AUDIO_FOLDER) / filename)
                    except Exception as e:
                ...
                # Display player (only if file exists)
                ...
                with col_download:
                    if filepath and Path(filepath).exists():
                        with open(filepath, 'rb') as f:
                            st.download_button(
                                f"💾 Save {output_format}",
                                f,
                                filename,
                                f"audio/{final_ext}",
                                key=f"dl_{st.session_state.last_task_id}_{i}"  # Unique key per track
                            )
                
                # Cover generation button
                cover_key = f"cover_{clip_id}"
                if st.button(f"🎨 Generate Cover", key=cover_key):
                    with st.spinner("Creating cover art..."):
                        conn = get_db()
                        cursor = conn.cursor()
                        cursor.execute("SELECT cover_images FROM generations WHERE clip_id = ?", (clip_id,))
                        ...
                        if result and result[0]:
                            ...
                            cover_task_id = generate_cover(st.session_state.api_key, clip_id)
                            if cover_task_id:
                                while True:
                                    ...
                                    if cover_status.get('status') == 'SUCCESS':
                                        ...
                                        if images:
                                            ...
                                            cursor.execute(
                                                ...
                                                (json.dumps(cover_paths), clip_id)
                                            )
                                            conn.commit()
                                            conn.close()
