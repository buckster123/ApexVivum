...
Advanced Visualizer Studio for Pi 5 - Phase 3 (Layout Controls)
"""
...
class EffectSettings:
    ...
    # NEW: Positioning and Sizing Controls
...
def render_position_preview(config: EffectSettings, preview_placeholder):
    ...
    bars_rect = Rectangle((bar_x, bar_y), bar_width, bar_height, 
                         facecolor='cyan', alpha=0.7, label='Spectrum Bars')
    ax.add_patch(bars_rect)
    ...
    if config.enable_waveform:
        ...
        wf_rect = Rectangle((wf_x, wf_y), wf_width, wf_height, 
                           facecolor=config.waveform_color, alpha=0.7, label='Waveform')
        ax.add_patch(wf_rect)
...
def music_visualizer_page():
    ...
    if "viz_config" not in st.session_state:
        ...
        if not hasattr(current_effects, 'config_version') or current_effects.config_version < 3:
            st.session_state.viz_config = migrate_config(st.session_state.viz_config)
            st.info("✅ Configuration migrated to new version with positioning controls!")
