...
Advanced Visualizer Studio for Pi 5 - Phase 5 (Multi-Band Waveforms)
"""
...
class EffectSettings:
    ...
    # Band-specific settings (2 or 3 bands)
    waveform_band_colors: List[str] = field(default_factory=lambda: ["#FF0000", "#00FF00", "#0000FF"])
    waveform_band_freq_splits: Tuple[int, int] = (250, 4000)  # Hz splits for 3-band mode
...
class AudioAnalyzer:
    ...
    def get_frame_features(self, time: float):
        ...
        return {
            ...
            'waveform_bands': self._get_waveform_bands_at(time)
        }
    ...
    def _get_waveform_at(self, time: float):
        """Get full waveform for single-band mode"""
    ...
    def _get_waveform_bands_at(self, time: float) -> Dict[str, np.ndarray]:
        """Extract multi-band waveforms using bandpass filters"""
        sample_pos = int(time * self.sr)
        window = self.sr // 10
        start = max(0, sample_pos - window // 2)
        end = min(len(self.y), start + window)
        audio_window = self.y[start:end]
        ...
        try:
            ...
            bands['bass'] = audio_window * 0.5
        
        # Mid: 250-4000 Hz
        try:
            bands['mid'] = self._filter_band(audio_window, 250, 4000)
        except:
            bands['mid'] = audio_window * 0.3
        
        # Treble: 4000-8000+ Hz
        try:
            bands['treble'] = self._filter_band(audio_window, 4000, min(8000, self.sr // 2))
        except:
            bands['treble'] = audio_window * 0.2
        
        # Normalize each band
        for band_name, band_data in bands.items():
            if len(band_data) > 0 and np.std(band_data) > 1e-6:
                bands[band_name] = (band_data - np.mean(band_data)) / np.std(band_data)
            else:
        ...
        if self.config.effects.enable_waveform:
            ...
            if self.config.effects.waveform_band_mode == WaveformBandMode.SINGLE:
                ...
                if self.config.effects.waveform_fill:
                    line, = ax_waveform.fill([], [], color=self.config.effects.waveform_color, alpha=0.3)
                else:
                    line, = ax_waveform.plot([], [], color=self.config.effects.waveform_color, 
                                            linewidth=self.config.effects.waveform_thickness)
                waveform_lines = [line]
            else:
                # Multi-band mode - create stacked or overlapping lines
                ...
                # Use fill for stacked look, plot for overlapping
...
def save_preset(name: str, config: EffectSettings):
    ...
    with open(preset_path, 'w') as f:
        ...
        for field in fields(config):
            ...
            if isinstance(value, (ColorScheme, PositionOption, AnchorOption, WaveformBandMode)):
...
def music_visualizer_page():
    ...
    if "viz_config" not in st.session_state:
        ...
        if not hasattr(current_effects, 'config_version') or current_effects.config_version < 6:
            st.session_state.viz_config = migrate_config(st.session_state.viz_config)
            st.info("✅ Configuration migrated to new version with multi-band waveforms!")
        ...
        with wf_col2:
            ...
            if st.session_state.viz_config.effects.enable_waveform:
                ...
                st.session_state.viz_config.effects.waveform_thickness = st.slider(
                    "Line Width", 0.5, 5.0, st.session_state.viz_config.effects.waveform_thickness,
                    help="Thickness of each band line"
                )
