...
Advanced Visualizer Studio for Pi 5 - Phase 4.1 (Intuitive UI & Smart Layouts)
"""
...
class EffectSettings:
    ...
    bar_thickness_pct: float = 15  # Renamed for clarity
    bar_length_pct: float = 80     # Renamed for clarity
...
def load_preset(name: str) -> EffectSettings:
    ...
    for enum_field in ['particle_color_scheme', 'bar_color_scheme', 'bar_position', 'waveform_position']:
        if enum_field in data and data[enum_field]:
            if isinstance(data[enum_field], str):
                if 'color' in enum_field.lower():
                    data[enum_field] = ColorScheme(data[enum_field])
                elif 'position' in enum_field.lower():
                    data[enum_field] = PositionOption(data[enum_field])
            else:
                # Handle enum objects
                data[enum_field] = ColorScheme(data[enum_field]['value']) if 'color' in enum_field else PositionOption(data[enum_field]['value'])
...
def render_position_preview(config: EffectSettings, preview_placeholder):
    ...
    # Draw waveform with orientation
    ...
    if config.enable_waveform:
        if wf_is_vertical:
            # Vertical waveform line
            ax.plot([wf_x + wf_thickness/2] * 2, [wf_y, wf_y + wf_length], 
                   color=config.waveform_color, linewidth=3, label='Waveform')
        else:
    ...
    orientation_text = f"Bars: {'Vertical' if bar_is_vertical else 'Horizontal'} | Wave: {'Vertical' if wf_is_vertical else 'Horizontal'}"
...
def music_visualizer_page():
    ...
    if "viz_config" not in st.session_state:
        ...
        if not hasattr(current_effects, 'config_version') or current_effects.config_version < 5:
            st.session_state.viz_config = migrate_config(st.session_state.viz_config)
            st.info("✅ Configuration migrated to new version with intuitive controls!")
    ...
    with tab2:
        ...
        # ==================== WAVEFORM SECTION ====================
        st.subheader("〰️ Waveform Overlay", divider="green")
        ...
        with wf_col2:
            ...
            if st.session_state.viz_config.effects.enable_waveform:
                st.session_state.viz_config.effects.waveform_color = st.color_picker(
                    "Color", st.session_state.viz_config.effects.waveform_color,
                    key="wf_color"
                )
