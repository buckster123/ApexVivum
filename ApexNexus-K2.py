...
if not API_KEY:
    st.error("KIMI_API_KEY not set in .env! Please add it and restart.")
...
default_prompts = {
    "default.txt": "You are Kimi, a highly intelligent AI assistant powered by Moonshot AI. Use provided tools when helpful.",
    "tools-enabled.txt": """You are Kimi, a highly intelligent AI assistant powered by Moonshot AI with access to sandboxed tools. Use tools only when explicitly needed or requested. Always confirm sensitive actions like writes. Invoke tools via structured calls, then incorporate results naturally.""",
}
...
def pip_install(venv_path: str, packages: list, upgrade: bool = False) -> str:
    ...
    if not safe_venv.startswith(os.path.abspath(SANDBOX_DIR)):
        return "Error: Venv path outside sandbox."
    pip_path = os.path.join(safe_venv, "bin", "pip")
...
def chat_log_analyze_embed(
    ...
    # Smart default: Use session state admin or fallback to 'andre' from bootstrap
    if user is None:
        user = st.session_state.get(
            "current_user", "andre"
        )  # Assumes session tracks user; fallback to admin
        # Alternative: user = os.getenv("DEFAULT_USER", "andre") if no Streamlit
    # Optional: Log the default for debugging/evolution
    if not st.session_state.get("original_user_provided"):
        # Insert to memory for tracking (optional, per bootstrap memory_system)
        memory_insert(
            "tool_defaults",
            {
                "tool": "chat_log_analyze_embed",
                "defaulted_user": user,
    ...
    result = c.fetchone()
    if not result:
        return f"Error: Chat log not found for user '{user}'."
    ...
    return f"Chat log {convo_id} analyzed and embedded as {mem_key} for user '{user}'."
...
def yaml_retrieve(query: str = None, top_k: int = 5, filename: str = None) -> str:
    ...
    try:
        existing_ids = set(col.get()["ids"])  # Get all existing embedded filenames
        if filename:
            ...
            query_emb = (
                embed_model.encode(query).tolist() if embed_model else [0.0] * 768
            )  # Dummy if no model
            results = col.query(
                query_embeddings=[query_emb],
                n_results=top_k,
        ...
        try:
            ...
            if func_name == "socratic_api_council":
                args["model"] = st.session_state.get(
                    "council_model_select", "kimi-k2-thinking"
                )
            if func_to_call:
...
def call_xai_api(
    ...
    def generate(current_messages):
        ...
        for iteration in range(max_iterations):
            main_count += 1
            logger.info(
                f"API call iteration {iteration+1}: Tools: {tool_count} | Council: {council_count} | Main: {main_count}"
            )
...
if "yaml_collection" not in st.session_state:
    try:
        ...
        st.session_state["yaml_cache"] = {}  # Empty; populate on demand
        st.session_state["yaml_ready"] = st.session_state.get("chroma_ready", False)
    except Exception as e:
